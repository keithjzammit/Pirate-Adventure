...
function simulateGames(iterations = 1000000) {
  isSimulationMode = true; // Enter simulation mode
  simulationResults = []; // Clear previous results
  simulationBalance = 1000000; // Start with an initial balance
...
function makeDecision(simulationBoard, gameResult) {
  ...
  //console.log("Risk Level: ",riskLevel, " Potential Reward: ",potentialReward, " Hint Cost: ", hintCost )
  if (riskLevel > 0.4) {
    return Math.random() < 0.5 ? 'cashout' : 'hint';
  } else if (riskLevel > 0.1 && hintCost < potentialReward * 0.8) {
    return 'hint';
  } else {
    return 'reveal';
  }
...
function displaySimulationSummary() {
  ...
  const totalMoneyLost = simulationResults.reduce((sum, game) => sum + (game.outcome === 'Lost' ? game.startPoints : 0), 0);
  ...
  const returnToPlayer = (totalMoneyWon/totalMoneyLost)*100; 
...
Simulation is not storing balance
Simulation has to pick at least one cell 
Simulation should not knows where the bombs are 
